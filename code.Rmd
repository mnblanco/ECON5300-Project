...
>>>>>>> 00cf9a0aa9de6f250e5900dd3cfb0b5c04f6984a
...
ss16ppr <- ss16ppr %>% select(WAGP, SEX, AGEP, RAC1P, RACWHT, RACBLK, RACASN, HISP, MAR, SCHL, ENG, COW, WKW, WKHP)
```
...
kable(ss16ppr %>%  group_by(RAC1P) %>% summarise(Count = n())  %>% arrange(desc(Count))) %>%
  kable_styling(bootstrap_options = "striped")
```
...
```{r}
ss16ppr  <- ss16ppr %>% mutate(RACASN = ifelse(RACASN == 1, "Yes", "No"))
kable(ss16ppr %>% group_by(RACASN) %>% summarise(Count = n())) %>%
  kable_styling(bootstrap_options = "striped")
```
