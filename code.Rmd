...
ss16ppr <- ss16ppr %>% select(ID, WAGP, SEX, AGEP, RAC1P, RACWHT, RACBLK, HISP, MAR, SCHL, ENG, COW, WKW, WKHP)
```
...
```{r}
kable(ss16ppr %>%  group_by(ENG) %>% summarise(Count = n())) %>%
  kable_styling(bootstrap_options = "striped")
```
...
ggplot(ss16ppr,  aes(RACOTHER, WAGP)) +
  geom_boxplot() +
  theme_minimal() +
  stat_summary(fun.data = fun_mean, geom="text", vjust=-0.7, size=3) +
  xlab("")
