...
For a black married male whose credit history meets guidline (GDLIN = 1), loan amount price is 100 (LOANPRC = 100) and other obligations as a percent of total income is none (OBRAT = 0), the log odds of loan approval is `r (round(1 / (1 + (exp(-(mylogit0$coefficients["(Intercept)"] + mylogit0$coefficients["GDLIN1"] + mylogit0$coefficients["BLACK1"] + 100 * mylogit0$coefficients["LOANPRC"] + mylogit0$coefficients["MARRIED1"] + mylogit0$coefficients["MALE1"])))), 4))*100`%
...
For a black married person whose credit history meets guidline (GDLIN = 1), loan amount price is 100 (LOANPRC = 100) and other obligations as a percent of total income is none (OBRAT = 0), the log odds of loan approval is `r (round(1 / (1 + (exp(-(mylogit1$coefficients["(Intercept)"] + mylogit1$coefficients["GDLIN1"] + mylogit1$coefficients["BLACK1"] + 100 * mylogit1$coefficients["LOANPRC"] + mylogit1$coefficients["MARRIED1"] )))), 4))*100`%
...
For a black person whose credit history meets guidline (GDLIN = 1), loan amount price is 100 (LOANPRC = 100) and other obligations as a percent of total income is none (OBRAT = 0), the log odds of loan approval is `r (round(1 / (1 + (exp(-(mylogit2$coefficients["(Intercept)"] + mylogit2$coefficients["GDLIN1"] + mylogit2$coefficients["BLACK1"] + 100 * mylogit2$coefficients["LOANPRC"])))), 4))*100`%
...
For every one unit change in `OBRAT`, the log odds of loan approval (versus non loan approval) decreases by `r abs(myprobit0$coefficients["OBRAT"])`.

For every one unit change in `LOANPRC`, the log odds of loan approval (versus non loan approval) decreases by `r abs(myprobit0$coefficients["LOANPRC"])`.

The log odds of loan approval for applicants that meet credit guidelines increases by  `r abs(myprobit0$coefficients["GDLIN1"])`.

The log odds of loan approval for married applicants increases by  `r abs(myprobit0$coefficients["MARRIED1"])`.

The log odds of loan approval for Black applicants decreases by  `r abs(myprobit0$coefficients["BLACK1"])`.

The log odds of loan approval for Hispanic applicants decreases by  `r abs(myprobit0$coefficients["HISPAN1"])`.

For a black married male whose credit history meets guidline (GDLIN = 1), loan amount price is 100 (LOANPRC = 100) and other obligations as a percent of total income is none (OBRAT = 0), the log odds of loan approval is `r (round(1- pnorm(-abs((myprobit0$coefficients["(Intercept)"] + myprobit0$coefficients["GDLIN1"] + myprobit0$coefficients["BLACK1"] + 100 * myprobit0$coefficients["LOANPRC"] + myprobit0$coefficients["MARRIED1"] + myprobit0$coefficients["MALE1"]))), 4))*100`%
...
For every one unit change in `OBRAT`, the log odds of loan approval (versus non loan approval) decreases by `r abs(myprobit1$coefficients["OBRAT"])`.

For every one unit change in `LOANPRC`, the log odds of loan approval (versus non loan approval) decreases by `r abs(myprobit1$coefficients["LOANPRC"])`.

The log odds of loan approval for applicants that meet credit guidelines increases by  `r abs(myprobit1$coefficients["GDLIN1"])`.

The log odds of loan approval for married applicants increases by  `r abs(myprobit1$coefficients["MARRIED1"])`.

The log odds of loan approval for Black applicants decreases by  `r abs(myprobit1$coefficients["BLACK1"])`.

The log odds of loan approval for Hispanic applicants decreases by  `r abs(myprobit1$coefficients["HISPAN1"])`.

For a black married person whose credit history meets guidline (GDLIN = 1), loan amount price is 100 (LOANPRC = 100) and other obligations as a percent of total income is none (OBRAT = 0), the log odds of loan approval is `r (round(1- pnorm(-abs((myprobit1$coefficients["(Intercept)"] + myprobit1$coefficients["GDLIN1"] + myprobit1$coefficients["BLACK1"] + 100 * myprobit1$coefficients["LOANPRC"] + myprobit1$coefficients["MARRIED1"]))), 4))*100`%
...
For every one unit change in `OBRAT`, the log odds of loan approval (versus non loan approval) decreases by `r abs(myprobit2$coefficients["OBRAT"])`.

For every one unit change in `LOANPRC`, the log odds of loan approval (versus non loan approval) decreases by `r abs(myprobit2$coefficients["LOANPRC"])`.

The log odds of loan approval for applicants that meet credit guidelines increases by  `r abs(myprobit2$coefficients["GDLIN1"])`.

The log odds of loan approval for Black applicants decreases by  `r abs(myprobit2$coefficients["BLACK1"])`.

The log odds of loan approval for Hispanic applicants decreases by  `r abs(myprobit2$coefficients["HISPAN1"])`.

For a black person whose credit history meets guidline (GDLIN = 1), loan amount price is 100 (LOANPRC = 100) and other obligations as a percent of total income is none (OBRAT = 0), the log odds of loan approval is `r (round(1- pnorm(-abs((myprobit2$coefficients["(Intercept)"] + myprobit2$coefficients["GDLIN1"] + myprobit2$coefficients["BLACK1"] + 100 * myprobit2$coefficients["LOANPRC"]))), 4))*100`%
