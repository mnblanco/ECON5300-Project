...
The plot shows that the distribution of the residuals are not approximately normally distributed.  The “thin tail” correspond to the first quantiles occurring at less than expected values and there is an outlier point. 

```{r echo=FALSE}
p[3]
```

The plot shows that the distribution of the residuals is not approximately normally distributed but skewed to the right.
...
$Log(Earning) = \beta_0 + Divorced * \beta_1 + NeverMarried * \beta_2 + Female * \beta_3 + RaceBlack * \beta_4 + RaceOther * \beta_5 +$ 
